...
| 2026-08-17 | Status check pulled Sequencing Track v3 (adopted this day, see dashboard) — case-study-driven JIT theory supersedes v2's phase ordering, since v2's Aug 18 target arrived with 0/14 case studies despite theory ~60% done. Same window: Topic 038 (Vertical vs Horizontal Scaling) initially saved as a thin stub despite the exact "tier shapes pace, not depth" correction already made once before (Topic 030) — caught and fixed, then codified directly into SYSTEM_DESIGN_MASTER_GUIDE.md so it can't recur a third time. Topic 039 (Replication) completed same window, clean 5/5 pass (motivations for replication, sync/async/semi-sync tradeoff, leader-follower vs multi-leader vs leaderless). | Topic 040 — Replication Lag & Read-Your-Writes |
...
| 2026-09-16 to 2026-09-17 | Topics 042 (Consistent Hashing, completed 2026-09-16) and 043 (Choosing a Shard Key, completed 2026-09-17) — completed in a parallel session (both 🔴 MUST, both confidence 5/5). 042 covered the hash-ring mechanism, why it minimizes reshuffling to ~1/N of keys, and virtual nodes for load balancing — one self-corrected slip describing virtual-node placement as deliberately "equidistant" instead of many independently-hashed random points. 043 covered shard-key selection criteria (cardinality, distribution, query alignment) and that a shard key can't fix hot-key problems (caching's job, not partitioning's) — clean on definitions, needed a nudge to volunteer the requested worked example. HLD table rows for both were backfilled this session since the parallel session's update had left them stale. | Topic 044 — Rebalancing & Resharding |
| 2026-09-19 | Topic 044 (Rebalancing & Resharding) — Completed (🟡 SKIM, condensed live pace per tier, full documentation retained), the final topic in the pre-TinyURL prerequisite batch (038-044 all done; only 098 Unique ID Generation remains before Case Study #1). Covered the two rebalancing organization styles (fixed over-partitioning vs dynamic split/merge), the live-migration mechanics (current owner stays authoritative during background copy, catch-up, atomic cutover), and the operator-gating guardrail against automatic rebalancing on transient node blips. Clean pass, plus a genuinely sharp follow-up question that caught an imprecision in the mentor's own explanation: a failure-triggered rebalance copies from a SURVIVING REPLICA, never the dead node itself (which is unreachable) — folded into the lesson file as its own section, directly reinforcing why per-partition replication (Topic 041) exists at all. Confidence: 5/5. | Topic 098 — Unique ID Generation (Snowflake, ULID) — last prerequisite before Case Study #1 (TinyURL) |
| 2026-09-21 | Topic 098 (Unique ID Generation: Snowflake, ULID) — Completed (🔴 MUST, 40m, Medium). **This closes out the entire pre-TinyURL prerequisite batch — Case Study #1 (TinyURL) begins next, learner-driven per Sequencing Track v3.** Covered why sharding breaks single auto-increment, random UUID v4 (122 bits of randomness, birthday-paradox collision math, B-tree fragmentation cost), Snowflake's [timestamp|machine|sequence] structure and its clock-jump-backward risk (NTP step corrections), and ULID as a zero-coordination timestamp-first alternative. Clean 3/3 pass on the checkpoint questions, plus two excellent self-driven follow-up questions: how UUID generation actually works (answered: 122 random bits + birthday paradox) and what an NTP clock-jump-backward actually is (answered with the mechanism and Snowflake's guard against it) — then a sharp clarifying push-back on why timestamp must lead the bit layout rather than machine ID, resolved with a worked numeric counterexample showing machine-ID-first only sorts within one machine's own stream, not globally. Confidence: 5/5. | Case Study #1 — TinyURL / URL Shortener (learner-driven whiteboard session) |
...
| 041 | Partitioning & Sharding | Completed | 2026-08-22 | 2026-08-22 | — | 0 | — | 2026-08-23 | 4 | Med-Hard | Credited per-shard replication only with read-scaling, omitted its primary availability/SPOF-prevention role — corrected |
| 042 | Consistent Hashing | Completed | 2026-09-16 | 2026-09-16 | — | 0 | — | 2026-09-17 | 5 | Med-Hard | Described virtual-node placement as "equidistant" spacing instead of many independently-hashed random points — self-corrected once the contradiction (requires knowing future node positions) was pointed out |
| 043 | Choosing a Shard Key | Completed | 2026-09-17 | 2026-09-17 | — | 0 | — | 2026-09-18 | 5 | Medium | Definitions were correct but needed a nudge to volunteer the requested conflicting-criteria worked example |
| 044 | Rebalancing & Resharding | Completed | 2026-09-19 | 2026-09-19 | — | 0 | — | 2026-09-20 | 5 | Medium | None — clean pass, plus a sharp follow-up question that caught an imprecision in the mentor's own explanation (copy source during a failure-triggered rebalance) |
...
| 098 | Unique ID Generation | Completed | 2026-09-21 | 2026-09-21 | — | 0 | — | 2026-09-22 | 5 | Medium | None — clean pass, plus a sharp clarifying question on timestamp-first vs machine-ID-first bit ordering (resolved with a worked counterexample) |
