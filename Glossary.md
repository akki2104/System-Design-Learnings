...
| Data-Structure Server | What Redis actually is — typed values (String/Hash/List/Set/ZSET/Stream/Bitmap/HLL/Geo) with atomic server-side operations, not a plain get/put cache | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `maxmemory-policy` | Redis's eviction setting. **Default `noeviction` rejects writes with an error when full**; `allkeys-lru` is the usual pure-cache choice; `volatile-*` variants evict only keys that have a TTL | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Lazy vs Active Expiry | Redis removes expired keys both on access (lazy) and via a background job sampling random TTL'd keys (active) — so an expired key can still occupy memory until sampled | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `UNLINK` vs `DEL` | `DEL` frees a collection synchronously (O(n), blocks the single command thread); `UNLINK` frees it in a background thread — the safe choice for big keys | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Hot Key | A single key taking disproportionate traffic. Sharding cannot fix it (one key = one slot = one shard = one thread) — fixes are an in-process L1 cache, key splitting, read replicas, or the CDN | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Big Key | A single very large collection whose O(n) access or deletion blocks every client on the instance and skews memory across shards | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Redis Sentinel | A quorum of monitor processes that detects a dead Redis primary, promotes a replica, and tells clients — **HA without sharding**; an alternative to Cluster, never layered with it | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `WAIT` (Redis) | Command asking for N replica acknowledgements before returning; reduces the window for losing an acknowledged write on failover but does **not** make Redis a CP system | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `min-replicas-to-write` | Redis setting making the primary refuse writes unless N replicas are within M seconds of lag — bounds data loss by trading availability for safety | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `appendfsync` | The AOF durability dial: `always` (safest/slowest) · `everysec` (default, lose ≤ ~1 s) · `no` (OS flushes) | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `MULTI` / `EXEC` | Redis's transaction: the queued batch executes with no interleaving, but there is **no rollback** on runtime errors and no read-then-branch inside it | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `WATCH` (Redis) | Optimistic concurrency control / compare-and-swap: `EXEC` returns nil if the watched key changed, and the **caller** must implement the retry loop | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Lua Script (`EVAL`) | Server-side script executing atomically — the practical way to do read-branch-write in Redis, which `MULTI/EXEC` cannot express. Must be short and (in Cluster) same-slot | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Pipelining | Batching many Redis commands into one network round trip for throughput. Provides **no** atomicity or isolation — it is not a transaction | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| `SET NX PX` | The one primitive behind three patterns: distributed lock, idempotency key, and cache-stampede mutex — atomic set-if-absent with an expiry | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Redlock | Antirez's multi-primary Redis locking algorithm (acquire on a majority of N independent instances). Contested by Kleppmann for relying on timing assumptions and lacking fencing tokens | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Fencing Token | A monotonically increasing number issued with a lock; the protected resource rejects lower tokens, so a stale lock holder cannot corrupt state — the real fix for correctness locks | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Efficiency Lock vs Correctness Lock | Efficiency: double execution is merely wasteful (a Redis lock suffices). Correctness: two holders corrupt data (needs fencing tokens, etcd/ZooKeeper, or idempotency) | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Redis Streams | Append-only in-memory log with consumer groups, `XACK`, a pending-entries list and `XCLAIM` redelivery — a queue with acknowledgement, bounded by RAM ("Kafka-lite") | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Redis Pub/Sub | Fire-and-forget, at-most-once fan-out with no persistence, no ack and no replay — an offline subscriber loses the message permanently | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| HyperLogLog | Probabilistic cardinality sketch: ≤ 12 KB per key at 0.81% standard error, counting up to 2^64 distinct items — unique-visitor counts without storing the members | [R01](Topics/R01_Redis_Consolidated_Module.md) |
| Fail Open vs Fail Closed | The decision every Redis-dependent component needs when Redis is unreachable: allow traffic through (rate limiter, cache) or reject it (billing quota, fraud gate) | [R01](Topics/R01_Redis_Consolidated_Module.md) |
